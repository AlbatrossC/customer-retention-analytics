...
def validate_columns(df: pd.DataFrame) -> None:
    required = set(DROP_COLUMNS + FEATURES)
...
def model_probabilities(model: XGBClassifier, calibrator: IsotonicRegression, x_data: pd.DataFrame) -> np.ndarray:
...
def permutation_feature_importance(model: XGBClassifier, x_test: pd.DataFrame, y_test: pd.Series) -> list[dict]:
...
def branch_and_colour_sweep(model: XGBClassifier, calibrator: IsotonicRegression, fixed_row: pd.Series) -> dict:
    ...
    branch_probs = model_probabilities(model, calibrator, prepare_x(pd.DataFrame(branch_rows)))
    ...
    colour_probs = model_probabilities(model, calibrator, prepare_x(pd.DataFrame(colour_rows)))
