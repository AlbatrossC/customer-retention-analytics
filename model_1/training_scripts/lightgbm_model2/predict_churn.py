...
def _load_artifacts():
    ...
    if missing:
        raise FileNotFoundError(
            f"LightGBM Model 2 artifacts not found: {missing}. "
            "Run python model_1/training_scripts/lightgbm_model2/train_lightgbm.py"
        )
...
def predict_churn(customer_data: dict, threshold: float | None = None) -> dict:
    ...
    customer_data = normalize_prediction_input(customer_data, metadata["features"])
    _validate_input(customer_data, metadata["features"])
    row = prepare_feature_row(customer_data, metadata, use_category_values=True)
