...
THRESHOLD_GRID = [0.03, 0.05, 0.06, 0.08, 0.10, 0.12, 0.15, 0.20, 0.25, 0.30, 0.40]
...
def category_values(df: pd.DataFrame) -> dict[str, list[str]]:
    return {feature: sorted(df[feature].dropna().astype(str).unique().tolist()) for feature in CATEGORICAL_FEATURES}


def prepare_x(df: pd.DataFrame, categories: dict[str, list[str]] | None = None) -> pd.DataFrame:
    ...
    for feature in CATEGORICAL_FEATURES:
        if categories:
            dtype = pd.CategoricalDtype(categories=categories[feature])
            x_data[feature] = x_data[feature].astype(str).astype(dtype)
        else:
...
def model_configs() -> list[dict]:
    ...
    candidates = [
        {"learning_rate": 0.015, "num_leaves": 15, "max_depth": 4, "min_child_samples": 80, "subsample": 0.85, "colsample_bytree": 0.85, "reg_alpha": 0.2, "reg_lambda": 5.0},
        {"learning_rate": 0.02, "num_leaves": 18, "max_depth": 4, "min_child_samples": 70, "subsample": 0.90, "colsample_bytree": 0.90, "reg_alpha": 0.1, "reg_lambda": 4.0},
        {"learning_rate": 0.015, "num_leaves": 24, "max_depth": 5, "min_child_samples": 90, "subsample": 0.85, "colsample_bytree": 0.80, "reg_alpha": 0.5, "reg_lambda": 6.0},
        {"learning_rate": 0.025, "num_leaves": 12, "max_depth": 3, "min_child_samples": 60, "subsample": 0.95, "colsample_bytree": 0.85, "reg_alpha": 0.0, "reg_lambda": 3.0},
        {"learning_rate": 0.02, "num_leaves": 31, "max_depth": 5, "min_child_samples": 120, "subsample": 0.80, "colsample_bytree": 0.80, "reg_alpha": 0.8, "reg_lambda": 8.0},
        {"learning_rate": 0.01, "num_leaves": 20, "max_depth": 4, "min_child_samples": 50, "subsample": 0.90, "colsample_bytree": 0.75, "reg_alpha": 0.3, "reg_lambda": 4.0},
    ]
    return [{**base, **candidate} for candidate in candidates]


def fit_best_model(x_train: pd.DataFrame, y_train: pd.Series, x_validation: pd.DataFrame, y_validation: pd.Series):
    best = None
    history = []
    for index, config in enumerate(model_configs(), start=1):
        ...
        validation_pr_auc = float(average_precision_score(y_validation, raw_probability))
        validation_roc_auc = float(roc_auc_score(y_validation, raw_probability))
        row = {
            "candidate": index,
            "validation_pr_auc_raw": validation_pr_auc,
            "validation_roc_auc_raw": validation_roc_auc,
            "best_iteration": int(model.best_iteration_ or config["n_estimators"]),
            "params": config,
        }
        history.append(row)
        if best is None or validation_pr_auc > best["validation_pr_auc_raw"]:
            best = {"model": model, **row}
    return best["model"], history


def model_probabilities(model: lgb.LGBMClassifier, calibrator: IsotonicRegression, x_data: pd.DataFrame) -> np.ndarray:
...
def branch_and_colour_sweep(
    ...
    branch_probs = model_probabilities(model, calibrator, prepare_x(pd.DataFrame(branch_rows), categories))
    ...
    colour_probs = model_probabilities(model, calibrator, prepare_x(pd.DataFrame(colour_rows), categories))
...
def main() -> None:
    ...
    categories = category_values(splits["train"])
    x_train = prepare_x(splits["train"], categories)
    x_validation = prepare_x(splits["validation"], categories)
    x_test = prepare_x(splits["test"], categories)
    y_train = splits["train"][TARGET]
    y_validation = splits["validation"][TARGET]
    y_test = splits["test"][TARGET]

    model, tuning_history = fit_best_model(x_train, y_train, x_validation, y_validation)

    validation_raw = model.predict_proba(x_validation)[:, 1]
    calibrator = IsotonicRegression(out_of_bounds="clip")
    calibrator.fit(validation_raw, y_validation)

    validation_probabilities = model_probabilities(model, calibrator, x_validation)
    test_probabilities = model_probabilities(model, calibrator, x_test)
    validation_metrics = evaluate(y_validation, validation_probabilities, args.threshold)
    test_metrics = evaluate(y_test, test_probabilities, args.threshold)
    ...
    feature_importance = permutation_feature_importance(model, x_test, y_test, args.permutation_repeats)
    decoy_ranks = {
    ...
    decoy_sweeps = branch_and_colour_sweep(model, calibrator, splits["test"].iloc[0], categories)

    metadata = {
        "model_name": "LightGBM Model 2 Customer Churn Classifier",
        "features": FEATURES,
    ...
    print("Split summary")
    for name, summary in metrics["split_summary"].items():
        print(
    ...
    for row in tuning_history:
        print(
            f"candidate={row['candidate']}, validation_pr_auc_raw={row['validation_pr_auc_raw']:.4f}, "
            f"validation_roc_auc_raw={row['validation_roc_auc_raw']:.4f}, best_iteration={row['best_iteration']}"
        )
    print("\nValidation threshold sweep using model probabilities")
    for row in validation_threshold_sweep:
        print(
            f"threshold={row['threshold']:.2f}, flagged={row['flagged_rows']}, "
            f"precision={row['precision']:.4f}, recall={row['recall']:.4f}"
        )
    print(f"\nDefault saved threshold: {args.threshold:.2f}")
