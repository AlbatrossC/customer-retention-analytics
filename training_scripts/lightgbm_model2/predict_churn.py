...
def _load_artifacts():
    ...
    if missing:
        raise FileNotFoundError(
            ...
            "Run python training_scripts/lightgbm_model2/train_lightgbm.py"
        )
...
def _prepare_row(customer_data: dict, metadata: dict) -> pd.DataFrame:
...
def predict_churn(customer_data: dict, threshold: float | None = None) -> dict:
    ...
    row = _prepare_row(customer_data, metadata)
